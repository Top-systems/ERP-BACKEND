...
const processarFiltro = async function({ valor, filtro }){ 
    ...
    const comecandoCom = await useDB({ 
        query: `SELECT * FROM cd_centro_custo  WHERE ${filtro.replaceAll("\\D", "")} like '${valor.replaceAll("\\D", "").toUpperCase()}%'`
    }); 
...
const pesquisarPorTexto = async function({ colunaBusca, textoBusca }){ 
    ...
    const pesquisa = await useDB({ 
        query: `SELECT * FROM cd_centro_custo WHERE UPPER(CAST(${colunaBusca} as text)) LIKE '%${textoBusca.toUpperCase().replaceAll("\\D", "")}%' ORDER BY ${colunaBusca} ASC`
    }); 

    /**
     *  String hql = "SELECT vo FROM CdCentrocusto vo"
