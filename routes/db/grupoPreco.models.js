...
const processarFiltro = async function({ valor, filtro }){ 
    ...
    const comecandoCom = await useDB({ 
        query: `SELECT * FROM cd_grupopreco  WHERE ${filtro.replaceAll("\\D", "")} like '${valor.replaceAll("\\D", "").toUpperCase()}%'`
    }); 
...
const pesquisarPorColuna = async function({ colunaBusca, textoBusca }){ 
    ...
    query: `SELECT * FROM Cd_Grupopreco  WHERE UPPER(CAST(${colunaBusca} as text)) LIKE '%${textoBusca.toUpperCase().replaceAll("\\D", "")}%' ORDER BY ${colunaBusca} ASC`
 }); 
